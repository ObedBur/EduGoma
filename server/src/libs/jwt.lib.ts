...
export const jwtVerify = (token: string) => jwt.verify(token, env.JWT_ACCESS_SECRET);
...
export const jwtVerifyRefresh = (token: string) => jwt.verify(token, env.JWT_REFRESH_SECRET);
